...
echo form_open('Admin/ajouterunproduit');
 ...
 echo  form_input('txtlibelle','',array('pattern'=>'[a-zA-Z0-9]+','title'=>'le produit  doit commencer par une lettre', 'required'=>'required','class'=>'form-control')).'<BR>';

 echo form_label('détail du produit :', 'lbldétail');

 echo  form_textarea('txtdetail', '',array('required'=>'required','class'=>'form-control')).'<BR>';
  echo  form_label('prixht :','lblprixht');
  echo  form_input('txtprixht', '',array('pattern'=>".{0,}",'title'=>'le prix doit est uniquement composé de chiffres','required'=>'required','class'=>'form-control')).'<BR>';

  echo  form_label('tauxtva :', 'lbltva');

  echo  form_input('txttva', '',array('pattern'=>".{0,10}" ,'title'=>'la tva doit est uniquement composé de chiffres','required'=>'required','class'=>'form-control')).'<BR>';
   ...
   echo form_input(array( 'name'=>'txtquantitestock','type'=>'number', 'min'=>'0','max'=>'1000','step'=>'1','required'=>'required','class'=>'form-control')).'<BR>';
...
echo "</select><br/>";
echo form_submit('boutonAjouter', 'ajouter',array('class'=>"btn btn-primary")).'<BR>';
echo form_close();
?>
</div>
</body>
